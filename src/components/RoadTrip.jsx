...
// The full 12-state corridor (including states driven through with no
// dedicated stop) gets the route tint; only stops get markers/panels.
const ROUTE_STATES = [
  ...new Set([...TRIP_STOPS.map((s) => s.state), 'IN', 'IL', 'KS', 'NM', 'TX', 'OK']),
]
    ...
    const ctx = gsap.context(() => {
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          start: 'top top',
          end: () => `+=${Math.max(2400, window.innerHeight * 2.5)}`,
          scrub: 1,
          pin: pinRef.current,
